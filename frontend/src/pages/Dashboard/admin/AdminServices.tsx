    ...
    <div className="px-4 w-full">
      ...
      <div className="flex flex-wrap gap-4 my-2 items-center justify-center 2xl:justify-start">
        {data.map((service: Service, index: number) => (
          <Box key={service._id || index} maxWidth="295px" className="overflow-auto">
            <Card size="2">
              <Inset clip="padding-box" side="top" pb="current">
                <img
                  src={service.image}
                  alt={service.title}
                  style={{ display: "block", objectFit: "cover", width: "100%", height: 140, backgroundColor: "var(--gray-5)" }}
                />
              </Inset>

              <Text as="p" size="4" className="text-center pb-2" weight="bold">{service.title}</Text>
              ...
              <Flex align="center" justify="between" gap="4" pt="4">
                <Button color="red" onClick={() => setDeleteServiceId(service._id)}>Delete</Button>
        ...
        <AlertDialog.Root open={!!deleteServiceId} onOpenChange={(open) => !open && setDeleteServiceId(null)}>
