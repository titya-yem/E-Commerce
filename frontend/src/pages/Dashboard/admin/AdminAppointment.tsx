...
import { Box, Flex, Heading, Select, Text, Button } from "@radix-ui/themes";
...
const AdminAppointments = () => {
  ...
  const updateStatus = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      const res = await axios.patch(`${import.meta.env.VITE_API_URL}/api/appointment/${id}/status`, { status });
      return res.data;
    },
    onSuccess: () => {
  ...
  if (!data || data.length === 0) return <Heading className="text-center py-10">No Appointments available</Heading>;

  const totalPages = Math.ceil(data.length / appointmentsPerPage);
  const currentAppointments = data.slice((currentPage - 1) * appointmentsPerPage, currentPage * appointmentsPerPage);

  return (
    <div className="px-6 lg:px-4 pb-6 lg:pb-0 w-full">
      <h2 className="text-xl lg:text-2xl xl:w-3xl text-center md:text-left py-5 font-medium">Appointments</h2>

      <Box className="p-2 rounded-md bg-white overflow-x-auto h-[590px] max-h-[600px]">
        <Box className="overflow-x-auto">

          {/* Header: only show on XL */}
          {isXl && (
            <div
          ...
          {currentAppointments.map((appointment: AppointmentTypes) => {
            const formattedTime = new Date(`1970-01-01T${appointment.time}`).toLocaleTimeString([], {
              hour: "numeric",
              minute: "2-digit",
              hour12: true,
            });
