...
export const getAllComments = async (req: Request, res: Response) => {
  try {
    const comments = await Comment.find({ status: "approved" }).populate("userName", "userName");
...
export const updateComment = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const updatedComment = await Comment.findByIdAndUpdate(id, updateData, { new: true }).populate("userName", "userName email");
...
// Delete comment (user deletes own)
...
export const updateCommentStatus = async (req: AuthRequest, res: Response) => {
  ...
  try {
    const updatedComment = await Comment.findByIdAndUpdate(id, { status }, { new: true }).populate("userName", "userName email");
